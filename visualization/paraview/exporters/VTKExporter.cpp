...
void VTKExporter::add_timestep_to_series(const std::shared_ptr<CoffeeBed>& bed,
    ...
    flow_filename << time_series_base_filename_ << "_flow_" 
                  << std::setfill('0') << std::setw(6) << timestep << ".vts";
...
void VTKExporter::finalize_time_series() {
    ...
    std::cout << "Finalized time series with " << time_series_entries_.size() 
              << " timesteps in " << time_series_collection_file_ << std::endl;
}
...
void VTKExporter::write_vtu_point_data(std::ofstream& file,
                                      const std::vector<std::shared_ptr<ICoffeeParticle>>& particles) {
    file << "      <PointData>\n";
    ...
    for (const auto& particle : particles) {
        file << "          " << std::fixed << std::setprecision(6) << particle->get_extraction_state() << "\n";
    }
    file << "        </DataArray>\n";
...
void VTKExporter::write_vts_header(std::ofstream& file, size_t nx, size_t ny, size_t nz, 
                                  double dx, double dy, double dz) {
    file << "<?xml version=\"" << VTK_XML_VERSION << "\"?>\n";
    file << "<VTKFile type=\"StructuredGrid\" version=\"1.0\" byte_order=\"" << VTK_BYTE_ORDER << "\">\n";
    file << "  <StructuredGrid WholeExtent=\"0 " << (nx-1) << " 0 " << (ny-1) << " 0 " << (nz-1) << "\">\n";
    file << "    <Piece Extent=\"0 " << (nx-1) << " 0 " << (ny-1) << " 0 " << (nz-1) << "\">\n";
}

void VTKExporter::write_vts_points(std::ofstream& file, size_t nx, size_t ny, size_t nz,
                                  double dx, double dy, double dz) {
    file << "      <Points>\n";
    file << "        <DataArray type=\"Float32\" NumberOfComponents=\"3\" format=\"ascii\">\n";
...
void VTKExporter::write_vts_point_data(std::ofstream& file, 
    ...
    for (size_t j = 0; j < ny; j++) {
        for (size_t i = 0; i < nx; i++) {
            ...
            file << "          " << std::fixed << std::setprecision(6) << vx << " " << vy << " 0.0\n";
        }
    }
    file << "        </DataArray>\n";

...
public:
  ...
  PhysicsSolver(std::shared_ptr<CoffeeBed> bed,
                std::shared_ptr<IWaterFlow> flow, Parameters params);
...
private:
  void modify_local_flow(size_t i, size_t j, double particle_size,
                         double extraction_state);
  void update_particle_state(const std::shared_ptr<ICoffeeParticle> &particle,
                             double vx, double vy);

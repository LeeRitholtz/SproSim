...
CoffeeParticle3D::CoffeeParticle3D(double x, double y, double z, double radius)
    : position{x, y, z}, radius(radius), extraction_state(0.0), concentration(0.0),
...
double CoffeeParticle3D::get_local_packing_density(const std::vector<CoffeeParticle3D>& neighbors,
...
generate_random_3d_distribution(int num_particles, double bed_radius, double bed_depth,
    ...
    for (int i = 0; i < num_particles; i++) {
        ...
        double size_factor =
            (particle_size - size_range.first) / (size_range.second - size_range.first);
...
void simulate_gravity_settling(std::vector<CoffeeParticle3D>& particles,
                               double gravity_acceleration, double settling_time,
                               double time_step) {
...
int resolve_particle_overlaps(std::vector<CoffeeParticle3D>& particles, int max_iterations) {
                ...
                if (particles[i].overlaps_with(particles[j])) {
                    ...
                    if (distance > 0) {
                        double required_separation =
                            (particles[i].get_size() + particles[j].get_size()) / 2.0;
                        ...
                        particles[i].update_position_3d(-dx * move_distance, -dy * move_distance,
                                                        -dz * move_distance);
                        particles[j].update_position_3d(dx * move_distance, dy * move_distance,
                                                        dz * move_distance);
                    }
                }
            }
...
void apply_size_stratification(std::vector<CoffeeParticle3D>& particles,
    ...
    for (auto& particle : particles) {
        ...
        double depth_adjustment =
            stratification_strength * size_factor * 0.005; // Max 5mm adjustment

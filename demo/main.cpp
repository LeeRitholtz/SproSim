...
class EspressoBrewingDemo {
  ...
  private:
    std::shared_ptr<CoffeeBed> create_coffee_bed() {
        ...
        for (double size : particle_sizes) {
            for (int i = 0; i < particles_per_size; i++) {
                ...
                auto particle =
                    std::make_shared<CoffeeParticle>(x, y, size / 2.0); // radius = diameter/2
    ...
    void display_results(std::shared_ptr<CoffeeBed> bed, std::shared_ptr<WaterFlow> flow) {
